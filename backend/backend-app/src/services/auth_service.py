...
def token_required(f):
    ...
    def decorated(*args, **kwargs):
        ...
        try:
            ...
            # Admins, evaluadores, instructores y estudiantes requieren cuenta activa
            if current_user.rol in ['admin', 'evaluador', 'instructor', 'estudiante']:
...
def evaluador_required(f):
    ...
    def decorated(current_user, *args, **kwargs):
        if current_user.rol != 'evaluador':
            return jsonify({
                'success': False,
                'error': 'Acceso denegado. Se requiere rol de evaluador'
            }), 403
...
def admin_or_evaluador_required(f):
    """Decorador para verificar que el usuario es administrador o evaluador"""
    @wraps(f)
    def decorated(current_user, *args, **kwargs):
        if current_user.rol not in ['admin', 'evaluador']:
            return jsonify({
                'success': False,
                'error': 'Acceso denegado. Se requiere rol de administrador o evaluador'
            }), 403
