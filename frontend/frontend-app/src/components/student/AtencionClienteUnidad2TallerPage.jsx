...
const pasosProtocolo = [

  {
    ...
    respuestaCorrecta: 'opcion_recibir'
...

  {
    ...
    respuestaCorrecta: 'opcion_verificar'
...

  {
    ...
    respuestaCorrecta: 'opcion_responder'
...

  {
    ...
    respuestaCorrecta: 'opcion_cerrar'
...
const opcionesDisponibles = [

  {
    ...
    texto: 'Dar la bienvenida al cliente, escuchar su inconformidad y agradecer la comunicación.'
...

  {
    ...
    texto: 'Revisar el pedido, confirmar el retraso y clasificar el caso como reclamo.'
...

  {
    ...
    texto: 'Ofrecer una solución clara: reposición, devolución o compensación, explicando los pasos a seguir.'
...

  {
    ...
    texto: 'Confirmar satisfacción, registrar el caso y agradecer la retroalimentación.'

  }

];

const AtencionClienteUnidad2TallerPage = () => {

  const navigate = useNavigate();

  const [respuestas, setRespuestas] = useState(() => {

    const saved = localStorage.getItem('ac_u2_taller_respuestas');

    return saved ? JSON.parse(saved) : {};

  });

  const [opcionesDisponiblesState, setOpcionesDisponiblesState] = useState(() => {

    const saved = localStorage.getItem('ac_u2_taller_disponibles');
    ...
    return [...new Set(opcionesDisponibles.map(o => o.id))];

  });

  const [validado, setValidado] = useState(() => {

    const saved = localStorage.getItem('ac_u2_taller_validado');
...
useEffect(() => {
    ...
    localStorage.setItem('ac_u2_taller_respuestas', JSON.stringify(respuestas));

  }, [respuestas]);

useEffect(() => {

    localStorage.setItem('ac_u2_taller_disponibles', JSON.stringify(opcionesDisponiblesState));

  }, [opcionesDisponiblesState]);

useEffect(() => {

    localStorage.setItem('ac_u2_taller_validado', validado.toString());
...
const handleDrop = (e, pasoId) => {
    ...
    if (draggedOpcion) {
      ...
      const opcion = opcionesDisponibles.find(o => o.id === draggedOpcion);

      if (opcion) {

        // Remover del paso anterior si existe

        const nuevaRespuestas = { ...respuestas };

        Object.keys(nuevaRespuestas).forEach(key => {

          if (nuevaRespuestas[key] === draggedOpcion) {
...
// Agregar al nuevo paso

        nuevaRespuestas[pasoId] = draggedOpcion;

        setRespuestas(nuevaRespuestas);

// Remover de disponibles

        setOpcionesDisponiblesState(prev => prev.filter(id => id !== draggedOpcion));
...
const handleRemove = (pasoId) => {

    const opcionId = respuestas[pasoId];

    if (opcionId) {

      const nuevaRespuestas = { ...respuestas };

      delete nuevaRespuestas[pasoId];

      setRespuestas(nuevaRespuestas);

      setOpcionesDisponiblesState([...opcionesDisponiblesState, opcionId]);
...
pasosProtocolo.forEach(paso => {
      ...
      const opcionId = respuestas[paso.id];

      if (opcionId) {

        const esCorrecta = opcionId === paso.respuestaCorrecta;

        resultados[paso.id] = esCorrecta;
...
const reiniciar = () => {
    ...
    setOpcionesDisponiblesState([...new Set(opcionesDisponibles.map(o => o.id))]);
    ...
    localStorage.removeItem('ac_u2_taller_respuestas');

    localStorage.removeItem('ac_u2_taller_disponibles');

    localStorage.removeItem('ac_u2_taller_validado');

  };

const allPasosCompletos = () => {

    return pasosProtocolo.every(paso => respuestas[paso.id]);
...
try {
      ...
      navigate('/student/atencion-cliente/unidad2/cierre');
...
<div className="max-w-7xl mx-auto relative z-10">
          ...
          >
            ...
            >
              ...
              Taller: aplicando el protocolo de atención de quejas y reclamaciones
      ...
      <div className="max-w-7xl mx-auto px-8 py-6">
        ...
        <div className="mb-6">
          ...
          <p className="text-neutral-700 text-base leading-relaxed mb-4">
            ...
            Ahora que conocemos el protocolo de atención de quejas y reclamaciones, vamos a realizar el siguiente taller para reforzar la Unidad 2.
        ...
        <div className="mb-6">
          ...
          <div className="bg-yellow-100 border-l-4 border-yellow-500 rounded-lg p-4 mb-3">
            ...
            <p className="text-neutral-700 text-sm md:text-base mb-2">
              ...
              Deberás arrastrar cada concepto disponible en el lado derecho hacia su espacio correspondiente en el lado izquierdo para establecer a qué fase del protocolo de atención de quejas y reclamaciones corresponde.
...
{/* Tabla de Drag and Drop */}
        ...
        <div className="bg-white rounded-lg shadow-md overflow-hidden mb-6">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-0">
            ...
            {/* Columna Izquierda - Pasos del Protocolo */}

            <div className="bg-gray-100 p-4">

              <h4 className="text-lg font-bold mb-4 text-center text-neutral-900">Paso del protocolo</h4>

              <div className="space-y-3">

                {pasosProtocolo.map((paso) => {

                  const opcionId = respuestas[paso.id];

                  const opcion = opcionId ? opcionesDisponibles.find(o => o.id === opcionId) : null;

                  const isDragOver = dragOver === paso.id;

                  const esCorrecta = mostrarValidacion && resultadoValidacion && resultadoValidacion[paso.id];

                  const esIncorrecta = mostrarValidacion && resultadoValidacion && !resultadoValidacion[paso.id] && opcion;
...
return (
                    ...
                    >
                      ...
                      {opcion ? (
                        ...
                        <p className="text-sm text-gray-500 italic">Arrastra una opción aquí</p>
...
{/* Columna Derecha - Opciones Disponibles */}
            ...
            <div className="bg-gray-50 p-4">
              ...
              <h4 className="text-lg font-bold mb-4 text-center text-neutral-900">Opciones</h4>

              <div className="space-y-3">

                {opcionesDisponibles

                  .filter(opcion => opcionesDisponiblesState.includes(opcion.id))
                  ...
                  .map((opcion) => (
                    ...
                    >
                      ...
                      <p className="text-sm text-neutral-700">{opcion.texto}</p>

                    </div>

                  ))}

                {opcionesDisponiblesState.length === 0 && (

                  <div className="text-center text-gray-500 py-8">

                    <p className="text-sm">Todas las opciones han sido asignadas</p>
        ...
        {mostrarReflexion && (
          ...
          >
            ...
            <p className="text-neutral-700 text-sm md:text-base font-semibold mb-2">
              ...
              Esta es la reflexión final que le aparece al participante una vez culminado el taller:

            </p>

            <p className="text-neutral-800 text-base md:text-lg italic">

              "Lo esencial no es evitar las fallas, sino atenderlas con empatía, responsabilidad y soluciones claras. Cada reclamo bien gestionado fortalece la confianza y abre el camino hacia la mejora continua."
...
{/* Botones de acción */}
        ...
        <div className="flex justify-end gap-4 mt-8">
          ...
          <Button
            ...
            disabled={!allPasosCompletos() || (mostrarValidacion && !validado)}

            className={`px-8 py-4 flex items-center gap-2 ${!allPasosCompletos() || (mostrarValidacion && !validado)
          ...
          >
            ...
            {validado ? 'Continuar a Evaluación' : allPasosCompletos() ? 'Verificar respuestas' : 'Completa todos los pasos'}

            {!validado && allPasosCompletos() && !mostrarValidacion && (
      ...
      <div className="fixed bottom-8 left-8 z-40">
        ...
        <Button
          ...
          onClick={() => {
            ...
            navigate('/student/atencion-cliente/unidad2/desarrollo');
...
export default AtencionClienteUnidad2TallerPage;
