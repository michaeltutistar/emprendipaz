    ...
    const fetchEvidenciasExistentes = async () => {
        try {
            ...
            if (response.ok) {
                ...
                // No hay evidencias, esto es normal
            ...
            setError('Error al cargar evidencias existentes');
        }
    };
        ...
        return (
            <div className="p-6">
                <div className="text-center text-red-500">
                    ...
                    <p>Error: {error}</p>
                    <Button onClick={fetchEvidenciasExistentes} className="mt-4">Reintentar</Button>
                </div>
            </div>
        );
    }
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                <div className="p-4 border rounded-lg">
                                    <h4 className="font-medium mb-2">
                                        {evidenciasExistentes.tipo === 'formal' ? 'Matrícula Mercantil' : 'Publicaciones Redes'}
                                    </h4>
                                    <p className="text-sm text-gray-600">{evidenciasExistentes.archivo1_nombre}</p>
                                </div>
                                <div className="p-4 border rounded-lg">
                                    <h4 className="font-medium mb-2">
                                        {evidenciasExistentes.tipo === 'formal' ? 'Facturas' : 'Registro de Ventas'}
                                    </h4>
                            ...
                            <div className="flex gap-4">
                                <label className="flex items-center space-x-2">
                                    <input
                                        ...
                                        name="tipo"
                                        value="formal"
                                        checked={tipoEmprendimiento === 'formal'}
                                        onChange={(e) => setTipoEmprendimiento(e.target.value)}
                                        className="h-4 w-4"
                                    />
                                    <span>Sí, está formalizado</span>
                                </label>
                                <label className="flex items-center space-x-2">
                                    <input
                                        type="radio"
                                        name="tipo"
                                        value="informal"
                                        checked={tipoEmprendimiento === 'informal'}
                                        onChange={(e) => setTipoEmprendimiento(e.target.value)}
                                        className="h-4 w-4"
                                    />
                                    <span>No, es informal</span>
                                </label>
                            </div>
                        </div>
...
export default EvidenceUploadFixed;
